  ...
  const cerrarModal=()=>{
    ...
    const handleAnularReserva= async (id)=>{
        try{
          ...
          if(res.ok){
            setReservasActivas((prev)=>prev.filter((reserva)=>reserva.id!==id));
    ...
    const manejarSubmit=async(datos)=>{
      try{
        const url=modoModal==="crear" ? "/api/v1/reserva" : `/api/v1/reserva/${reservaSeleccionada.id}`;
        const method=modoModal === "crear" ? "POST" : "PATCH";
        const body= modoModal==="crear" ? JSON.stringify(datos) : JSON.stringify({plazas_reservadas: datos.plazas_reservadas});
